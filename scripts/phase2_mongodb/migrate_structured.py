...
def migrate_structured():
    ...
    pipeline = [
        ...
        # 2. Genres
        {"$lookup": {
            "from": "genres", "localField": "movie_id", "foreignField": "movie_id", "as": "g"
        }},

        # 3. Titres
        {"$lookup": {
            "from": "titles", "localField": "movie_id", "foreignField": "movie_id", "as": "t"
        }},
        ...
        {"$lookup": {
            ...
            "pipeline": [
                ...
                {"$lookup": {"from": "persons", "localField": "person_id", "foreignField": "person_id", "as": "p"}},
                {"$unwind": "$p"},
        ...
        {"$lookup": {
            ...
            "pipeline": [
                ...
                {"$lookup": {
                    ...
                    "pipeline": [{"$match": {"$expr": {"$and": [{"$eq": ["$movie_id", "$$mid"]}, {"$eq": ["$person_id", "$$pid"]}]}}}],
                    "as": "char"
                }},
                {"$project": {
                    "_id": 0, "person_id": 1, "name": "$p.name", "ordering": 1,
        ...
        {"$project": {
            ...
            "titles": {
                "$map": {
                    "input": "$t", "as": "tmp", "in": {"region": "$$tmp.region", "title": "$$tmp.title"}
                }
            }
        }},

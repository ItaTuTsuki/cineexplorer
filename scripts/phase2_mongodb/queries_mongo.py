...
def query_q1_filmography(db, actor_name):
    ...
    pipeline = [
        ...
        {"$lookup": {
            "from": "movies", "localField": "movie_id", "foreignField": "movie_id", "as": "m"
        }},
        {"$unwind": "$m"},
        {"$lookup": { # On ajoute le personnage
            ...
            "pipeline": [
                {"$match": {"$expr": {"$and": [{"$eq": ["$movie_id", "$$mid"]}, {"$eq": ["$person_id", "$$pid"]}]}}}
            ],
            "as": "char"
        }},
        {"$project": {
...
def query_q2_top_movies(db, genre, year_start, year_end, n):
    pipeline = [
        ...
        {"$lookup": {
            "from": "ratings", "localField": "movie_id", "foreignField": "movie_id", "as": "r"
        }},
        {"$unwind": "$r"},
        {"$sort": {"r.average_rating": -1, "r.num_votes": -1}},
        {"$limit": n},
        {"$project": {"title": "$m.title", "year": "$m.year", "rating": "$r.average_rating"}}
    ]
...
def query_q3_multi_roles(db):
    pipeline = [
        ...
        {"$match": {"count": {"$gt": 1}}},
        {"$lookup": {
            "from": "persons", "localField": "_id.pid", "foreignField": "person_id", "as": "p"
        }},
        {"$lookup": {
            "from": "movies", "localField": "_id.mid", "foreignField": "movie_id", "as": "m"
        }},
        {"$project": {
...
def query_q4_collaborations(db, actor_name):
    ...
    actor_movies = db.principals.distinct("movie_id", {"person_id": pid, "category": {"$in": ["actor", "actress"]}})

    pipeline = [
        {"$match": {"movie_id": {"$in": actor_movies}}}, # On filtre direct sur ces films
        {"$lookup": {
            "from": "persons", "localField": "person_id", "foreignField": "person_id", "as": "dir"
        }},
        {"$unwind": "$dir"},
        {"$group": {
            "_id": "$dir.name",
...
def query_q8_breakthrough(db):
    ...
    pipeline = [
        ...
        {"$lookup": {
            "from": "principals", "localField": "movie_id", "foreignField": "movie_id", "as": "cast"
        }},
        {"$unwind": "$cast"},
        ...
        {"$lookup": {
             "from": "persons", "localField": "_id", "foreignField": "person_id", "as": "p"
        }},
        {"$project": {"name": {"$arrayElemAt": ["$p.name", 0]}, "hit": "$hit_movie_id"}}
    ]
